...
if (!function_exists('geshi_highlight')) {
    /**
     * Easy way to highlight stuff. Behaves just like highlight_string|简单的方法来突出东西。行为就像highlight_string一样
     *
     * @param string The code to highlight|要强调的代码
     * @param string The language to highlight the code in|用于突出显示代码的语言
     * @param string The path to the language files. You can leave this blank if you need|语言文件的路径。如果你需要，你可以留下这个空白
     *               as from version 1.0.7 the path should be automatically detected|从版本1.0.7开始，应自动检测路径
     * @param boolean Whether to return the result or to echo|是 return 结果还是 echo
     * @return string The code highlighted (if $return is true)|代码突出显示
     * @since 1.0.2
     */
    function geshi_highlight($string, $language, $path = null, $return = false)
    {
        ...
        if ($return) {
            /**
             * 为了在Parsedown中调用geshi.php，略做修改
             * @author TabKey9 <Admin@tlip.cn>
             * @date        2018-04-12
             * @anotherdate 2018-04-12 03:39:13
             */
                return $geshi->parse_code();
                // return '<code>' . $geshi->parse_code() . '</code>';//原版
        }
